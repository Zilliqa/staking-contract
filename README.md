...
| `SwapDelegator` | `initiator: ByStr20, new_deleg: ByStr20` | To change ownership of existing stakes, rewards, pending withdrawals, etc. to another address `initiator` is the address of the delegator who wants to transfer his/her stakes. `new_deleg` is the address of the recipient that would be receiving all the staked amount, rewards, pending withdrawals, etc, of the `initiator` (original owner). Please be cautious of the `new_deleg` address as the change is irreversible. | <center>:x:</center> | :heavy_check_mark: |
...
| `SwapDelegator(new_deleg: ByStr20)` | `SwapDelegator(initiator: ByStr20, new_deleg: ByStr20)` |
